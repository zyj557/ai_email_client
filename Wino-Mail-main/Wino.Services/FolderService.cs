...
public class FolderService : BaseDatabaseService, IFolderService
{
    ...
    #region AI Folder Management

    /// <summary>
    /// 为指定账户创建AI分类文件夹
    /// </summary>
    public async Task CreateAIFoldersForAccountAsync(Guid accountId)
    {
        var account = await _accountService.GetAccountAsync(accountId);
        if (account == null) return;

        var aiFolderTypes = new[]
        {
            SpecialFolderType.AIImportant,
            SpecialFolderType.AIWork,
            SpecialFolderType.AIPersonal,
            SpecialFolderType.AIFinance,
            SpecialFolderType.AIShopping,
            SpecialFolderType.AITravel,
            SpecialFolderType.AISocial,
            SpecialFolderType.AINews,
            SpecialFolderType.AIEducation,
            SpecialFolderType.AIHealth,
            SpecialFolderType.AITechnology,
            SpecialFolderType.AIEntertainment
        };

        var folderNames = new Dictionary<SpecialFolderType, string>
        {
            { SpecialFolderType.AIImportant, "重要邮件" },
            { SpecialFolderType.AIWork, "工作相关" },
            { SpecialFolderType.AIPersonal, "个人邮件" },
            { SpecialFolderType.AIFinance, "财务相关" },
            { SpecialFolderType.AIShopping, "购物相关" },
            { SpecialFolderType.AITravel, "旅行相关" },
            { SpecialFolderType.AISocial, "社交媒体" },
            { SpecialFolderType.AINews, "新闻资讯" },
            { SpecialFolderType.AIEducation, "教育相关" },
            { SpecialFolderType.AIHealth, "健康医疗" },
            { SpecialFolderType.AITechnology, "技术相关" },
            { SpecialFolderType.AIEntertainment, "娱乐相关" }
        };

        foreach (var folderType in aiFolderTypes)
        {
            var existingFolder = await GetSpecialFolderByAccountIdAsync(accountId, folderType);
            if (existingFolder == null)
            {
                var aiFolder = new MailItemFolder
                {
                    Id = Guid.NewGuid(),
                    MailAccountId = accountId,
                    FolderName = folderNames[folderType],
                    SpecialFolderType = folderType,
                    RemoteFolderId = $"AI_{folderType}_{accountId}",
                    IsSticky = true,
                    ShowUnreadCount = true,
                    IsSynchronizationEnabled = false, // AI文件夹不需要同步
                    IsHidden = false
                };

                await InsertFolderAsync(aiFolder);
                _logger.Information("Created AI folder {FolderName} for account {AccountId}", aiFolder.FolderName, accountId);
            }
        }
    }
    ...
    /// 获取AI重要邮件数量
    /// </summary>
    public async Task<int> GetAIImportantMailCountAsync(Guid accountId)
    {
        var query = new Query(nameof(MailCopy))
                    .Join(nameof(MailItemFolder), $"{nameof(MailCopy)}.FolderId", $"{nameof(MailItemFolder)}.Id")
                    .Where($"{nameof(MailItemFolder)}.MailAccountId", accountId)
                    .Where($"{nameof(MailCopy)}.AIImportanceScore", ">=", 4)
                    .Where($"{nameof(MailCopy)}.IsAIAnalyzed", true)
                    .SelectRaw("count(DISTINCT MailCopy.Id)");
    ...
    /// 获取AI分类邮件数量
    /// </summary>
    public async Task<int> GetAICategoryMailCountAsync(Guid accountId, SpecialFolderType categoryType)
    {
        if (!IsAICategoryFolder(categoryType)) return 0;

        var aiCategory = GetAICategoryFromFolderType(categoryType);
        
        var query = new Query(nameof(MailCopy))
                    .Join(nameof(MailItemFolder), $"{nameof(MailCopy)}.FolderId", $"{nameof(MailItemFolder)}.Id")
                    .Where($"{nameof(MailItemFolder)}.MailAccountId", accountId)
                    .Where($"{nameof(MailCopy)}.AICategory", (int)aiCategory)
                    .Where($"{nameof(MailCopy)}.IsAIAnalyzed", true)
                    .Where($"{nameof(MailCopy)}.IsAISpam", false)
                    .SelectRaw("count(DISTINCT MailCopy.Id)");
    ...
    /// 获取垃圾邮件数量
    /// </summary>
    public async Task<int> GetAISpamMailCountAsync(Guid accountId)
    {
        var query = new Query(nameof(MailCopy))
                    .Join(nameof(MailItemFolder), $"{nameof(MailCopy)}.FolderId", $"{nameof(MailItemFolder)}.Id")
                    .Where($"{nameof(MailItemFolder)}.MailAccountId", accountId)
                    .Where($"{nameof(MailCopy)}.IsAISpam", true)
                    .Where($"{nameof(MailCopy)}.IsAIAnalyzed", true)
                    .SelectRaw("count(DISTINCT MailCopy.Id)");
    ...
    private bool IsAICategoryFolder(SpecialFolderType folderType)
    {
        return folderType >= SpecialFolderType.AIWork && folderType <= SpecialFolderType.AIEntertainment;
    }

    private AIMailCategory GetAICategoryFromFolderType(SpecialFolderType folderType)
    {
        return folderType switch
        {
            SpecialFolderType.AIWork => AIMailCategory.Work,
            SpecialFolderType.AIPersonal => AIMailCategory.Personal,
            SpecialFolderType.AIFinance => AIMailCategory.Finance,
            SpecialFolderType.AIShopping => AIMailCategory.Shopping,
            SpecialFolderType.AITravel => AIMailCategory.Travel,
            SpecialFolderType.AISocial => AIMailCategory.Social,
            SpecialFolderType.AINews => AIMailCategory.News,
            SpecialFolderType.AIEducation => AIMailCategory.Education,
            SpecialFolderType.AIHealth => AIMailCategory.Health,
            SpecialFolderType.AITechnology => AIMailCategory.Technology,
            SpecialFolderType.AIEntertainment => AIMailCategory.Entertainment,
